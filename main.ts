...
let dynamicNumber:number = Math.floor(Math.random() * 10) + 1;
...
let firstVar: number = 25;
let secondVar:number = 45;
function callVariables(...variables:number[]){
    return variables;
};
...
let anonynousFunction = (name:string)=>{ console.log(chalk.bgBlueBright(`My name is ${name}`)) };

anonynousFunction("xyz");


function anonynous(name:string){
